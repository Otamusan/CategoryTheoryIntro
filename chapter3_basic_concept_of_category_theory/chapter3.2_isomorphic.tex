\subsection{同型}\label{chap-3.2-isomorphic}
  ...
  \begin{prop}[同型による元の対応]\label{prop-isomorphic-and-elements}
  ...
  \begin{prop}[集合の圏の同型による元の対応]\label{prop-isomorphic-on-sets}
    集合の圏$\cat{Set}$において$A\cong B\iff \varDelta a\sim \varDelta b$なる$\varDelta a,\varDelta b$が一対一対応。
  \end{prop}  