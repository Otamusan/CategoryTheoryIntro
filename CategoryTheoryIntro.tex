...
\begin{document}
  ...
  次に同型と射集合の関係性について見ていく。

  \begin{prop}[同型射となる共変射写像]
    ある圏$\cat{C}$においてある二対象$A\cong B$が同型であるならば、任意の対象$X$に対し$\arset{C}{X}{A}\cong\arset{C}{X}{B}$が成り立つ。
  \end{prop}

  \begin{proof}
    同型射を$\mor{i}{A}{B}$、$\mor{i^{-1}}{B}{A}$として、共変射写像\[\mor{\arset{C}{X}{i}}{\arset{C}{X}{A}}{\arset{C}{X}{B}}\]と\[\mor{\arset{C}{X}{i^{-1}}}{\arset{C}{X}{B}}{\arset{C}{X}{A}}\]もまた同型射になることを示す。すなわち、\[\arset{C}{X}{i}\circ\arset{C}{X}{i^{-1}}=id_{\arset{C}{X}{B}}\]\[\arset{C}{X}{i^{-1}}\circ\arset{C}{X}{i}=id_{\arset{C}{X}{A}}\]を示せばよい。
    ...
    \begin{align*}
      ...
      &=id_B\cong f&\text{(同型射の定義)}\\
      &=f&\text{(恒等射の定義)}\\
      &=id_{\arset{C}{X}{B}}(f)&\text{(恒等射の定義)}
    \end{align*}
    よって$\arset{C}{X}{i}\circ\arset{C}{X}{i^{-1}}=id_{\arset{C}{X}{B}}$が示せた。同様に$\arset{C}{X}{i^{-1}}\circ\arset{C}{X}{i}=id_{\arset{C}{X}{A}}$も示せる。
    
    よって$\mor{\arset{C}{X}{i}}{\arset{C}{X}{A}}{\arset{C}{X}{B}}$と$\mor{\arset{C}{X}{i^{-1}}}{\arset{C}{X}{B}}{\arset{C}{X}{A}}$もまた同型射となるため、$\arset{C}{X}{A}\cong\arset{C}{X}{B}$が成り立つ。
		\begin{center}
			\begin{tikzpicture}[auto]
				\node (x) at (-1.5, 0) {$X$};
				\node (x2) at (-1.5, -1.5) {$X$};
  ...
  これはつまり、対象$A$からの射、$A$への射は対象$B$への射、$B$からの射と一対一対応をするということである。今はまだ触れないが、この対応は単に全単射であるだけでなく、圏論の基本的な操作である射の合成を保つ。
  

  ここから先では同型による射集合の元の一対一対応は、対象の同型と同じく同一視することにする。例を上げると、
  同型射$\mor{i}{A}{B}$、$\mor{i^{-1}}{B}{A}$による同型$A\cong B$に対し、射$\mor{f}{X}{A}$のコドメイン$A$を同型によって$B$に置き換えるが、その時$f$に対応する射は$i\circ f$であるが、$\mor{f}{X}{B}$と表記する。
  ...
  \begin{define}[評価射]
		...
		\end{align*}
		と定義する。もしくは
    これは元$b$に写像$f$を適用する操作を写像にしたものである。またこのような射は任意の対象$A,B$に対して個別に存在する。
		厳密な証明は行わないが評価射は実際に写像になる。
	\end{define}
  同型$A\cong\arset{Set}{1}{A}$、$B\cong\arset{Set}{1}{B}$が成り立つから、評価射は\[\mor{ev_{A,B}}{\arset{Set}{B}{A}\times \arset{Set}{1}{B}}{\arset{Set}{1}{A}} \]とも表すことができるが、これは合成を行う写像の特別な場合である。
    ...
    同様にこのような射は任意の対象$A,B$に対して個別に存在し実際に写像になる。
	\end{define}
  ...
  余評価射$\mor{ce_{A,B}}{A}{\arset{Set}{B}{A\times B}}$は任意の対象$A$,$B$に対して存在するのであったが、この対象$B$に終対象$1$を当てはめる。すると
  \begin{align*}
    \mor{&ce_{A,1}}{A}{\arset{Set}{1}{A\times 1}}\\
    &ce_{A,1}(a)=g_a\\
    &g_a(*)=\tuple{a,*}
  \end{align*}
  のような射になる。ここで、$A\times 1\cong A$が成り立つから、\[\mor{ce_{A,1}}{A}{\arset{Set}{1}{A}}\]と置き換えることができる。少し複雑ではあるが、この置き換えは$A\times 1\cong A$の同型射\[\mor{\pi_A}{A\times 1}{A}\]を合成しているのではなく、この同型により導出される$\arset{Set}{1}{A\times 1}\cong\arset{Set}{1}{A}$の同型射である\[\mor{\arset{Set}{1}{\pi_A}}{\arset{Set}{1}{A\times 1}}{\arset{Set}{1}{A}}\]を合成している。

  つまり、実際の置き換えられた射は$\arset{Set}{1}{\pi_A}\circ ce_{A,1}$である。まずはこの射が$\mor{i}{A}{\arset{Set}{1}{A}}$と等しいことを確かめる。$A$の任意の元$a$において、
  \begin{align*}
    \arset{Set}{1}{\pi_A}(ce_{A,1}(a))&=\arset{Set}{1}{\pi_A}(g_a)&\text{(余評価射の定義)}\\
  ...
  \end{align*}
  となる。よって$\pi_A\circ g_a=e_a$が成り立ち、\[\arset{Set}{1}{\pi_A}\circ ce_{A,1}(a)=e_a=i(a)\]となる。これが任意の$A$の元$a$で成り立つから\[\arset{Set}{1}{\pi_A}\circ ce_{A,1}=i\]となる。これで$\mor{i}{A}{\arset{Set}{1}{A}}$を余評価射で表すことができた。

  次に$\mor{i^{-1}}{\arset{Set}{1}{A}}{A}$を評価射で表す。

  同様に評価射$\mor{ev_{A,B}}{\arset{Set}{B}{A}\times B}{A}$の対象$B$に終対象$1$を当てはめると、
  \[\mor{ev_{A,1}}{\arset{Set}{1}{A}\times 1}{A}\]となる。ここで同型$A\times 1\cong A$は任意の対象$A$で成り立つから、$A$を$\arset{Set}{1}{A}$に当てはめた$\arset{Set}{1}{A}\times 1\cong \arset{Set}{1}{A}$も成り立つ。この同型によって$\arset{Set}{1}{A}\times 1$を置き換える。すなわち$\arset{Set}{1}{A}\times 1\cong \arset{Set}{1}{A}$の同型射\[\mor{\tuple{id_{\arset{Set}{1}{A}},!_{\arset{Set}{1}{A}}}}{\arset{Set}{1}{A}}{\arset{Set}{1}{A}\times 1}\]を合成した、\[\mor{ev_{A,1}\circ\tuple{id_{\arset{Set}{1}{A}},!_{\arset{Set}{1}{A}}}}{\arset{Set}{1}{A}}{A}\]が$\mor{i^{-1}}{\arset{Set}{1}{A}}{A}$と等しいことを確認する。

  $\arset{Set}{1}{A}$の任意の元$e_a$において、
  \begin{align*}
    ev_{A,1}\circ\tuple{id_{\arset{Set}{1}{A}},!_{\arset{Set}{1}{A}}}(e_a)&=ev_{A,1}(\tuple{e_a,*})&\text{(射の対の分配則)}\\
    &=e_a(*)&\text{(評価射の定義)}\\
    &=i^{-1}(e_a)&\text{($A\cong \arset{Set}{1}{A}$の同型射の定義)}
  \end{align*}
  よって\[ev_{A,1}\circ\tuple{id_{\arset{Set}{1}{A}},!_{\arset{Set}{1}{A}}}=i^{-1}\]が成り立つ。よって$\mor{i^{-1}}{\arset{Set}{1}{A}}{A}$を余評価射で表すことができた。

  次に評価射、余評価射の持つ基本的な性質を確かめていく。

  \begin{prop}[冪の三角恒等式1]
    任意の対象$A,B$において、\[\arset{Set}{B}{ev_{A,B}}\circ ce_{\arset{Set}{B}{A},B}=id_{\arset{Set}{B}{A}}\]が成り立つ。
  \end{prop}

  \begin{proof}
    任意の射$\mor{f}{B}{A}$に対して、
    \begin{align*}
      \arset{Set}{B}{ev_{A,B}}(ce_{\arset{Set}{B}{A},B}(f))&=\arset{Set}{B}{ev_{A,B}}(g_f)&\text{(余評価射の定義)}\\
      &=ev_{A,B}\circ g_f&\text{(共変射写像の定義)}\\
    \end{align*}
    が成り立つ。さらに対象$B$の任意の元$b$に対して、
    \begin{align*}
      ev_{A,B}\circ g_f(b)&=ev_{A,B}(\tuple{f,b})&\text{(余評価射の定義)}\\
      &=f(b)&\text{(評価射の定義)}\\
    \end{align*}
    となる。よって\[\arset{Set}{B}{ev_{A,B}}\circ ce_{\arset{Set}{B}{A},B}=ev_{A,B}\circ g_f=f\]が任意の$f$で成り立つから、\[\arset{Set}{B}{ev_{A,B}}\circ ce_{\arset{Set}{B}{A},B}=id_{\arset{Set}{B}{A}}\]となる。
  \end{proof}

  \begin{prop}[冪の三角恒等式2]
    任意の対象$A,B$において、\[ev_{(A\times B),B}\circ(ce_{A,B}\times id_B)\]が成り立つ。
  \end{prop}

  \begin{proof}
    対象$A\times B$の任意の元$\tuple{a,b}$に対して、
    \begin{align*}
      ev_{(A\times B),B}((ce_{A,B}\times id_B)\tuple{a,b})=
      &=ev_{(A\times B),B}(\tuple{ce_{A,B}(a),b})&\text{(射の積の定義)}\\
      &=ev_{(A\times B),B}(\tuple{g_a,b})&\text{(余評価射の定義)}\\
      &=g_a(b)&\text{(評価射の定義)}\\
      &=\tuple{a,b}&\text{(余評価射の定義)}
    \end{align*}
    となる。よって\[ev_{(A\times B),B}\circ(ce_{A,B}\times id_B)\]が成り立つ。
  \end{proof}
  \section{関手}
