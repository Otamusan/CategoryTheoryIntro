...
\begin{document}
	...
	\begin{define}
		...
		\begin{quote}
			...
			で表せる。また厳密には二項演算$\circ$は任意の対象$A,B,C$の組み合わせごとに存在する。
  ...
  \end{proof}
	...
	\end{proof}
  なぜ同型が圏論内で同一とみなせるかは現段階では説明できないが、概略を示す。まず圏論では特別視したい対象に性質を持たせる時、その性質をその対象をドメイン、コドメインとする射によって記述する。これは次に扱う普遍性の一種、積対象と終対象の定義で確認してほしい。

  つまり、対象が等価であることを示すには対象の周りの射が同じ性質を持つことを示せば良い。また射は圏内の操作である射の合成という操作によって初めて区別されるから、射が等価であるかは射の合成によって区別できるかを調べれば良い。
  ...
  \begin{define}[評価射]
	...
	\end{define}
  同型$A\cong\arset{Set}{1}{A}$、$B\cong\arset{Set}{1}{B}$が成り立つから、評価射は\[\mor{ev}{\arset{Set}{B}{A}\times \arset{Set}{1}{B}}{\arset{Set}{1}{A}} \]とも表すことができるが、これは合成を行う写像の特別な場合である。
  ...
  \subsection{同型と周りの射の対応}
  
  次に同型の定義で紹介した、なぜ同型である二対象が圏論内で同一視できるのかについて説明していく。
  ...
  さらに共変射写像と反変射写像による射集合の同型を統合する。

  \begin{prop}[同型射となる射写像]
    \[A\cong A',B\cong B'\Longrightarrow\arset{C}{A}{B}\cong\arset{C}{A'}{B'}\]
  \end{prop}
  \begin{proof}
    $A\cong A'$ならば$\arset{C}{A}{B}\cong\arset{C}{A'}{B}$であり、$B\cong B'$ならば$\arset{C}{A'}{B}\cong\arset{C}{A'}{B'}$である。よって同型の推移律より、$\arset{C}{A}{B}\cong\arset{C}{A'}{B'}$が成り立つ。

    もう少し丁寧に見ていくと、それぞれの同型射を$\mor{i}{A}{A'}$、$\mor{i^{-1}}{A'}{A}$、$\mor{j}{B}{B'}$、$\mor{j^{-1}}{B'}{B}$とした時、
    \[\mor{\arset{C}{i}{B'}\circ\arset{C}{A}{j}}{\arset{C}{A}{B}}{\arset{C}{A'}{B'}}\]と\[\mor{\arset{C}{A}{j^{-1}}\circ\arset{C}{i^{-1}}{B'}}{\arset{C}{A'}{B'}}{\arset{C}{A}{B}}\]が同型射の一つになる。
    \begin{center}
			\begin{tikzpicture}[auto]
				\node (cab) at (0, 0) {$\arset{C}{A}{B}$};
				\node (ca'b) at (0, -1.5) {$\arset{C}{A'}{B}$};
				\node (catc) at (2, 1) {$\cat{Set}$};
				\draw[->,transform canvas={xshift=-2pt}] (cab) to node[swap]{$\arset{C}{i}{B}$}(ca'b);
        \draw[->,transform canvas={xshift=2pt}] (ca'b) to node[swap]{$\arset{C}{i^{-1}}{B}$}(cab);
        \node (cab') at (4, 0) {$\arset{C}{A}{B'}$};
				\node (ca'b') at (4, -1.5) {$\arset{C}{A'}{B'}$};
				\draw[->,transform canvas={xshift=-2pt}] (cab') to node[swap]{$\arset{C}{i}{B'}$}(ca'b');
        \draw[->,transform canvas={xshift=2pt}] (ca'b') to node[swap]{$\arset{C}{i^{-1}}{B'}$}(cab');

        \draw[->,transform canvas={yshift=-2pt}] (cab) to node[swap]{$\arset{C}{A}{j}$}(cab');
        \draw[->,transform canvas={yshift=2pt}] (cab') to node[swap]{$\arset{C}{A}{j^{-1}}$}(cab);
        \draw[->,transform canvas={yshift=-2pt}] (ca'b) to node[swap]{$\arset{C}{A'}{j}$}(ca'b');
        \draw[->,transform canvas={yshift=2pt}] (ca'b') to node[swap]{$\arset{C}{A'}{j^{-1}}$}(ca'b);
			\end{tikzpicture}
		\end{center}
    またこの同型射は任意の射$\mor{f}{A}{B}$に対して
    \[\arset{C}{i}{B'}\circ\arset{C}{A}{j}(f)=j\circ f\circ i^{-1}\]
    \[\arset{C}{A}{j^{-1}}\circ\arset{C}{i^{-1}}{B'}(f)=j^{-1}\circ f\circ i\]となるような写像である。
  \end{proof}

  \begin{define}[射の同値]
    同型射$\mor{i}{A}{A'}$、$\mor{i^{-1}}{A'}{A}$、$\mor{j}{B}{B'}$、$\mor{j^{-1}}{B'}{B}$に対して、ある射$\mor{f}{A}{B}$に対応する射$f'$を\[f'=j\circ f\circ i^{-1}\]とし、これを$f\sim f'$と表記し、同型射$i,j$による射の同値と呼ぶことにする。
    \begin{center}
			\begin{tikzpicture}[auto]
				\node (a) at (-1.5, 0) {$A$};
				\node (a') at (-1.5, -1.5) {$A'$};
        ...
        \draw[->] (a') to node[swap]{$f'$}(b');
				...
				\draw[->,transform canvas={xshift=-2pt}] (a) to node[swap]{$i$}(a');
        \draw[->,transform canvas={xshift=2pt}] (a') to node[swap]{$i^{-1}$}(a);
        \draw[->,transform canvas={xshift=-2pt}] (b) to node[swap]{$j$}(b');
        \draw[->,transform canvas={xshift=2pt}] (b') to node[swap]{$j^{-1}$}(b);
			\end{tikzpicture}
		\end{center}
    また同型$\arset{C}{A}{B}\cong\arset{C}{A'}{B'}$によって、任意の射$f$は$f\sim f'$となるような射$f'$と一対一対応をする。
  \end{define}
  この関係$\sim$が同値関係であることは同型が同値関係であることから直ぐにわかる。
  \begin{prop}[射の合成による射の同値の保存]
    射$\mor{f}{A}{B}$、$\mor{f'}{A'}{B'}$、$\mor{g}{B}{C}$、$\mor{g'}{B'}{C'}$が$f\sim f'$、$g\sim g'$である時、$g\circ f\sim g'\circ f'$が成り立つ。
  \end{prop}
  \begin{proof}
    同型射をそれぞれ、$\mor{i}{A}{A'}$、$\mor{i^{-1}}{A'}{A}$、$\mor{j}{B}{B'}$、$\mor{j^{-1}}{B'}{B}$、
  \end{proof}
  %\begin{proof}
  %  $\sim$は同値関係であることを証明する。
  %  \begin{quote}
	%		\begin{description}
	%			\item[反射律] 任意の射$\mor{f}{A}{B}$に対して恒等射$id_A,id_B$を同型射と見なした同型$A\cong A,B\cong B$を考えると、
	%			\[id_B\circ f\circ id_A=f,\,id_B\circ f\circ id_A=f\]となるから$f\sim f$が成り立つ。 
  ...
  %          \draw[->] (a') to node[swap]{$f$}(b');
  %  
  %          \draw[->,transform canvas={xshift=-2pt}] (a) to node[swap]{$id_A$}(a');
  %          \draw[->,transform canvas={xshift=2pt}] (a') to node[swap]{$id_A$}(a);
  %          \draw[->,transform canvas={xshift=-2pt}] (b) to node[swap]{$id_B$}(b');
  %          \draw[->,transform canvas={xshift=2pt}] (b') to node[swap]{$id_B$}(b);
  %        \end{tikzpicture}
  %      \end{center}

	%			\item[対称律] $\mor{i}{A}{A'}$と、逆射$\mor{i^{-1}}{A'}{A}$、に対して$f\sim f'$が成り立つとき、$\mor{i}{A}{A'}$を逆射とみなすことで$f'\sim f$となる。
	%			\item[推移律] $\mor{i}{A}{A'}$と、逆射$\mor{i^{-1}}{A'}{A}$による関係$f\sim f'$、$\mor{j}{A'}{A''}$と、逆射$\mor{j^{-1}}{A''}{A'}$による関係$f'\sim f''$、が成り立つとき、新しい同型射$\mor{j\circ i}{A}{A''}$、$\mor{j^{-1}\circ i^{-1}}{A''}{A}$が構成できる。これにより$f\sim f''$が成り立つ。
  %    \end{description}
  %  \end{quote} 
  %\end{proof}

  \subsection{評価射、余評価射の応用}
  次に集合の圏での同型$A\cong \arset{Set}{1}{A}$の証明に使用した同型射を評価射、余評価射として表そうと思う。ただし、この話題は現段階ではかなり複雑な話になってしまう。もし射集合の持つ根本的な性質に今の段階で興味がないのであれば飛ばしてもらっても構わない。
  ...
  \begin{align*}
    ...
    &g_a(*)=\tuple{a,*}
  \end{align*}
  のような射になる。ここで、$A\times 1\cong A$が成り立つから、\[\mor{ce}{A}{\arset{Set}{1}{A}}\]と置き換えることができる。少し複雑ではあるが、この置き換えは$A\times 1\cong A$の同型射\[\mor{\pi_A}{A\times 1}{A}\]を合成しているのではなく、この同型により導出される$\arset{Set}{1}{A\times 1}\cong\arset{Set}{1}{A}$の同型射である\[\mor{\arset{Set}{1}{\pi_A}}{\arset{Set}{1}{A\times 1}}{\arset{Set}{1}{A}}\]を合成している。

  つまり、実際の置き換えられた射は$\arset{Set}{1}{\pi_A}\circ ce$である。まずはこの射が$\mor{i}{A}{\arset{Set}{1}{A}}$と等しいことを確かめる。$A$の任意の元$a$において、
  \begin{align*}
  ...
  次に評価射、余評価射の持つ基本的な二つの性質を確かめていく。

  \begin{prop}[冪の三角恒等式1]
    任意の対象$A,B$において、\[\arset{Set}{B}{ev}\circ ce=id_{\arset{Set}{B}{A}}\]が成り立つ。
    \begin{center}
			\begin{tikzpicture}[auto]
        ...
        (setba2);
        \draw[->] (setba1) to node{$\arset{Set}{B}{ev}$}(setbbab);
        \draw[->] (setbbab) to node{$ce$}(setba2);
			\end{tikzpicture}
		\end{center}
  \end{prop}
  ...
  \begin{proof}
    任意の射$\mor{f}{B}{A}$に対して、
    \begin{align*}
      \arset{Set}{B}{ev}(ce(f))&=\arset{Set}{B}{ev}(g_f)&\text{(余評価射の定義)}\\
      &=ev\circ g_f&\text{(共変射写像の定義)}\\
    \end{align*}
    が成り立つ。さらに対象$B$の任意の元$b$に対して、
    \begin{align*}
      ev\circ g_f(b)&=ev(\tuple{f,b})&\text{(余評価射の定義)}\\
      &=f(b)&\text{(評価射の定義)}\\
    \end{align*}
    となる。よって\[\arset{Set}{B}{ev}\circ ce=ev\circ g_f=f\]が任意の$f$で成り立つから、\[\arset{Set}{B}{ev}\circ ce=id_{\arset{Set}{B}{A}}\]となる。
  \end{proof}
  ...
  \begin{proof}
    ...
    \begin{align*}
      ev((ce\times id_B)\tuple{a,b})=
      &=ev(\tuple{ce(a),b})&\text{(射の積の定義)}\\
      &=ev(\tuple{g_a,b})&\text{(余評価射の定義)}\\
      &=g_a(b)&\text{(評価射の定義)}\\
  ...
  最後にこれらの性質を用いて同型$A\cong \arset{Set}{1}{A}$を証明する。

  \section{関手}
	これまでの議論は殆どが一つの圏の上で行われてきたが、これからはある圏とまた別の圏の関係性を中心に考えていく。
	\begin{define}
	...
	また反変関手$\functor{F}{C}{D}$は共変関手$\functor{F}{C^{op}}{D}$とみなせる。
		...
		\end{center}
	\subsection{Hom関手}
  以前集合の圏で扱った共変射写像、反変射写像は実はHom関手と呼ばれる関手の射関数によって移された射である。まずは改めてこれらの写像の性質を確認していこうと思う。
