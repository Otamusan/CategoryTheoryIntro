...
\begin{document}
	...
	\begin{define}[スライス圏]
		...
		\begin{itemize}
			\item (1)
			$\cat{C}$の任意の対象$X$において$\mor{f}{A}{X}$となるような射を$\cat{C}/A$の対象$S_f$とする。
			\item (2)
	...
	これを圏論の言葉で記述する。
	\begin{prop}
		$A\cong A' \Longleftrightarrow$任意の対象$X$、任意の射$\mor{f}{A}{X}$に対してある射$\mor{f'}{A'}{X}$が存在して、同型射を$i,i^{-1}$とすると$f'\circ i=f$、$f\circ i^{-1}=f'$が成り立つ。
		\begin{center}
			\begin{tikzpicture}[auto]
				\node (a) at (0, 0) {$a$};
				\node (a') at (0, -2) {$a'$};
	...
	\begin{proof}
		$(\Longleftarrow)$ $f=f'\circ i$、$f'=f\circ i^{-1}$にそれぞれを代入することで$f'\circ i\circ i^{-1}=f'$、$f\circ i^{-1}\circ i=f$が成り立つ。これらは$i,i^{-1}$に対して任意の$X,f$について成り立つから、$X,f$に$A,id_A$と$A',id_A'$を当てはめると、それぞれ$i\circ i^{-1}=id_A$、$i^{-1}\circ i=id_A'$が成り立つ。
		...
		$(\Longrightarrow)$ 任意の対象$X$と射$\mor{f}{A}{X}$に対して$f$に対応する射$f'$を$i^{-1}\circ f$とすると、$f'=f\circ i^{-1}$となり、両辺に$i$を合成して$f'\circ i=f\circ i^{-1}\circ i$となり、$A\cong A'$の定義より$f'\circ i=f$よって求められる条件を満たした$f'$の存在を示せた。
	\end{proof}
