...
\begin{document}
  ...
  次にある関手$\functor{F}{C}{D}$から$\functor{\varDelta D}{C}{D}$への自然変換$\nat{\alpha}{F}{\varDelta D}$を考えると、対象$A$に対する成分は$\mor{\alpha_A}{FA}{\varDelta D(A)}$であるから$\mor{\alpha_A}{FA}{D}$となり、射のドメインのみを対象で量化することができることが分かる。
  \begin{center}
    \begin{tikzpicture}[auto]
      \node (A) at (0, 1) {$A$};
      \node (B) at (2, 1) {$B$};
  ...
  \begin{prop}[関手圏の積]
    \begin{center}
      $(F\times G,\pi_F,\pi_G)$が$\funccat{C}{D}$における$F,G$の積対象$\Longrightarrow$圏$\cat{C}$の任意の対象$A$に対して$(F\times G)A\cong FA\times GA$\\
      ※完備性を仮定しない場合本当に成り立つ…？
    \end{center}
    ここで積の普遍性について確認しておく。
    任意の自然変換$\nat{\alpha}{H}{F}$、$\nat{\beta}{H}{G}$に対してある自然変換$\nat{\tuple{\alpha,\beta}}{H}{F\times G}$が存在し、\\
    \[\pi_F\cdot\tuple{\alpha,\beta}=\alpha,\ \pi_G\cdot\tuple{\alpha,\beta}=\beta\]を満たす。同様に、
    \[\pi_F\cdot\gamma=\alpha,\ \pi_G\cdot\gamma=\beta\]を満たす$\nat{\gamma}{X}{F\times G}$が存在する時、$\tuple{\alpha,\beta}=\gamma$である。\\
    証明の方針としては、各$A$に対して$(F\times G)A$が$FA,\ GA$に対する積であることを示し、証明することにする。\\
    \begin{center}
      \begin{tikzpicture}[auto]
        \node (x) at (2, 3) {$\cat{D^C}$};
        \node (a) at (0, 0) {$F$};
        \node (b) at (4, 0) {$G$};
        \node (ab) at (2, 0) {$F\times G$};
        \node (x) at (2, 2) {$H$};
        \draw[double,double equal sign distance,-implies] (ab) to node {$\pi_F$}(a);
        \draw[double,double equal sign distance,-implies] (ab) to node[swap] {$\pi_G$}(b);
        \draw[double,double equal sign distance,-implies] (x) to node[swap] {$\alpha$}(a);
        \draw[double,double equal sign distance,-implies] (x) to node {$\beta$}(b);
        \draw[double,double equal sign distance,-implies] (x) to node {$\tuple{\alpha,\beta}$}(ab);
      \end{tikzpicture}
    \end{center}
    ここでわかりやすさを優先して任意の関手$H$に任意の定関手$\varDelta X$を取り、成分の図式を考えると、
    \begin{center}
      \begin{tikzpicture}[auto]
        \node (x) at (2, 3) {$\cat{D}$};
        \node (a) at (0, 0) {$FA$};
        \node (b) at (4, 0) {$GA$};
        \node (ab) at (2, 0) {$(F\times G)A$};
        \node (x) at (2, 2) {$X$};
        \draw[->] (ab) to node {$(\pi_F)_A$}(a);
        \draw[->] (ab) to node[swap] {$(\pi_G)_A$}(b);
        \draw[->] (x) to node[swap] {$\alpha_A$}(a);
        \draw[->] (x) to node {$\beta_A$}(b);
        \draw[->] (x) to node {$\tuple{\alpha,\beta}_A$}(ab);
      \end{tikzpicture}
    \end{center}
