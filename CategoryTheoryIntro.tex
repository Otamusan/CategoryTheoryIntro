...
\begin{document}
	...
	\begin{proof}
		...
		また任意の元$\mor{a}{1}{A}$に対し\begin{align*}
	...
	\begin{define}[積圏]
		...
		\begin{quote}
			\begin{description}
				...
				すなわち、圏$\cat{A}$と圏$\cat{B}$の任意の対象$A,B$の対$\tuple{A,B}$が$\cat{A\times B}$の対象であり、紛らわしくないように\[\tuple{A,B}=[A,B]\]と表記する。

				また$\obj{A}\times\obj{B}$は直積集合であり、集合の圏の積対象であるが、対象$[A,B]$そのものは積対象ではないことに注意してほしい。今回積とみなすのは対象ではなく圏の方である。
				\item[射]　任意の対象$[A,B],[A,B]$に対してその射集合をそれぞれ\[\arset{(A\times B)}{[A,B]}{[A,B]}=\arset{A}{A}{A'}\times\arset{B}{B}{B'}\]と定義する。

				すなわち、圏$\cat{A}$の射$\mor{f}{A}{A'}$と圏$\cat{B}$の射$\mor{g}{B}{B'}$の対\[\mor{\tuple{f,g}}{[A,B]}{[A',B']}\]が$\cat{A\times B}$の射であり、対象と同様に\[\tuple{f,g}=[f,g]\]と表記する。
				...
				\begin{center}
					\begin{tikzpicture}[auto]
						...
						\draw[->] (a) to node{$f$}(a');
						\draw[->] (b) to node{$g$}(b');
						\draw[->] (ab) to node{$[f,g]$}(ab');
						\node (cata) at (0, 1) {$\cat{A}$};
						\node (catb) at (1.5, 1) {$\cat{B}$};
						\node (catb) at (3.5, 1) {$\cat{A\times B}$};
					\end{tikzpicture}
				\end{center}
				\item[射の合成] 射$\mor{[f,g]}{[A,B]}{[A',B']}$と$\mor{[f',g']}{[A',B']}{[A'',B'']}$の合成射\[\mor{[f',g']\circ[f,g]}{[A,B]}{[A',B']}\]を\[[f',g']\circ[f,g]=[f'\circ f,g'\circ g]\]と定義する。
				\begin{center}
					\begin{tikzpicture}[auto]
						\node (a) at (0, 0) {$A$};
						\node (a') at (0, -2) {$A'$};
						...
						\draw[->] (b) to node{$g$}(b');
						\draw[->] (b') to node{$g'$}(b'');
						\draw[->] (ab) to node[swap]{$[f,g]$}(ab');
						\draw[->] (ab') to node[swap]{$[f',g']$}(ab'');
						\draw[->,bend left =40] (ab) to node{$[f'\circ f,g'\circ g]$}(ab'');
						\node (cata) at (0, 1) {$\cat{A}$};
						\node (catb) at (1.5, 1) {$\cat{B}$};
						\node (catb) at (3.5, 1) {$\cat{A\times B}$};
					\end{tikzpicture}
				\end{center}
				\item[恒等射の存在]　対象$[A,B]$の恒等射$id_{[A,B]}$を\[id_{[A,B]}=[id_A,id_B]\]と定義する。
				\item[結合律]$([f'',g'']\circ[f',g'])\circ[f,g]=[f'',g'']\circ([f',g']\circ[f,g])$を示せばよい。
				\begin{align*}
					([f'',g'']\circ[f',g'])\circ[f,g]&=[(f''\circ f')\circ f,(g''\circ g')\circ g]&\text{(積圏の射の合成の定義)}\\
					&=[f''\circ (f'\circ f),g''\circ (g'\circ g)]&\text{(圏$\cat{A,B}$の結合則)}\\
					&=[f'',g'']\circ([f',g']\circ[f,g])&\text{(積圏の射の合成の定義)}
				\end{align*}
				よって成り立つ。
				\item[単位元律]任意の対象$[A,B]$と恒等射$id_{[A,B]}$、任意の射$\mor{[f,g]}{[X,Y]}{[A,B]}$、$\mor{[f',g']}{[A,B]}{[X',Y']}$において\[id_{[A,B]}\circ[f,g]=[f,g],\ [f',g']\circ id_{[A,B]}=[f',g']\]が成り立つことを示せばよい。
				\begin{align*}
					id_{[A,B]}\circ[f,g]&=[id_A,id_B]\circ[f,g]&\text{(圏$\cat{A\times B}$の恒等射の定義)}\\
					&=[id_A\circ f,id_B\circ g]&\text{(圏$\cat{A\times B}$の射の合成の定義)}\\
					&=[f,g]&\text{(圏$\cat{A,B}$の単位元律)}\\
					[f',g']\circ id_{[A,B]}&=[f',g']\circ[id_A,id_B]&\text{(圏$\cat{A\times B}$の恒等射の定義)}\\
					&=[f'\circ id_A,g'\circ id_B]&\text{(圏$\cat{A\times B}$の射の合成の定義)}\\
					&=[f',g']&\text{(圏$\cat{A,B}$の単位元律)}
				\end{align*}
				よって単位元律が成り立つ。
			\end{description}
	...
	\begin{define}[射影関手]
		...
		\begin{quote}
			\begin{description}
				...
				すなわち$\Pi_A([f,g])=f$となるような写像である。
				\item[恒等射の保存] $\Pi_A(id_{A\times B})=\Pi_A([id_A,id_B])=id_A$より恒等射を保つ
				\item[射の合成の保存]
				\begin{align*}
					\Pi_A([f',g']\circ[f,g])&=\Pi_A([f'\circ f, g'\circ g])&\text{($\cat{A\times B}$の射の合成の定義)}\\
					&=f'\circ f&\text{(射関数の定義)}\\
					&=\Pi_A([f',g'])\circ\Pi_A([f,g])&\text{(射関数の定義)}
				\end{align*}
				よって射の合成を保つ。
			\end{description}
		\end{quote}
	...
	\begin{define}[関手の対]
		...
		\begin{quote}
			\begin{description}
				...
				\end{align*}
				を対象関数とする。
				すなわち圏$\cat{X}$の対象$X$に対して$\tuple{F,G}(X)=[FX,GX]$となるような写像である。
				\item[射関数]
				...
				\end{align*}
				を射関数とする。
				すなわち射$\mor{f}{X}{X'}$に対して\[\mor{\tuple{F,G}(f)=[Ff,Gf]}{[FX,GX]}{[FX',GX']}\]となるような写像である。
				...
				\item[恒等射の保存]
				\begin{align*}
					\tuple{F,G}(id_X)&=[F(id_X),G(id_X)]&\text{(射関数の定義)}\\
					&=[id_{FX},id_{GX}]&\text{($F,G$の恒等射の保存)}\\
					&=id_{[FX,GX]}&\text{(積圏の恒等射の定義)}
				\end{align*}
			よって恒等射を保つ
				\item[射の合成の保存]
				\begin{align*}
					\tuple{F,G}(f'\circ f)&=[F(f'\circ f),G(f'\circ f)]&\text{(射関数の定義)}\\
					&=[Ff'\circ Ff, Gf'\circ Gf]&\text{($F,G$の射の合成の保存)}\\
					&=[Ff',Gf']\circ[Ff,Gf]&\text{(積圏の射の合成の定義)}\\
	...
	\begin{define}[双関手]
		...
		また積圏の任意の対象$[A,B]$に対し、\[F([A,B])=F(A,B)\]と略記する。
		さらに積圏の射$\mor{[f,id_B]}{[A,B]}{[A,B']}$において\[F([f,id_B])=F(f,B)\]と表記する。
	\end{define}
	...
	\begin{center}
		\begin{tikzpicture}[auto]
			...
			\node (b') at (4.5, 1) {$B'$};
			\node (ab) at (2, 0) {$[A,B]$};
			\node (a'b) at (2, -2.5) {$[A',B]$};
			\node (ab') at (4.5, 0) {$[A,B]$};
			\node (a'b') at (4.5, -2.5) {$[A',B']$};
			...
			\draw[->] (a) to node[swap]{$f$}(a');
			\draw[->] (b) to node{$g$}(b');
			\draw[->] (ab) to node[swap]{$[f,id_B]$}(a'b);
			\draw[->] (ab') to node{$[f,id_{B'}]$}(a'b');
			\draw[->] (ab) to node{$[id_A,g]$}(ab');
			\draw[->] (a'b) to node[swap]{$[id_{A'},g]$}(a'b');
	...
	\begin{define}[双積関手]
		...
		\begin{quote}
			\begin{description}
				...
				\item[射関数] 二対象$[A,B],[A',B']$に対する射関数を任意の射$\mor{[f,g]}{[A,B]}{[A',B']}$に対して\[(-\times -)_{[A,B],[A',B']}(f,g)=f\times g\]と定義する。
				\begin{center}
					\begin{tikzpicture}[auto]
						\node (ab) at (0, 0) {$[A,B]$};
						\node (a'b') at (0, -2) {$[A',B']$};
				...
				\item[恒等射の保存] $(-\times -)(id_{[A,B]})=id_{(-\times-)(A,B)}$を示せばよい。
				\begin{align*}
					(-\times -)(id_{[A,B]})&=id_A\times id_B&\text{(射関数の定義)}\\
					&=[id_A\circ\pi_A,id_B\circ\pi_B]&\text{(射の積の定義)}\\
					&=id_{A\times B}&\text{(射影射の対)}\\
					&=id_{(-\times-)(A,B)}&\text{(対象関数の定義)}
				\end{align*}
				よって恒等射を保つ。
				\item[射の合成の保存] $(-\times -)(f',g')\circ(-\times-)(f,g)=(-\times-)([f',g']\circ[f,g])$を示せばよい。
				\begin{align*}
					...
					&=(-\times-)([f'\circ f,g'\circ g])\\
					&=(-\times-)([f',g']\circ[f,g])&\text{(積圏の射の合成の定義)}
				\end{align*}
				よって射の合成を保つ。
			\end{description}
		\end{quote}
	\end{define}
	対象の積と圏の積が紛らわしいが、まず圏の積の対象$[A,B]$は圏$\cat{C}$の$A$と$B$の積が存在しなくとも定義することができ、その点で対象の積$A\times B$より一般的な概念だと考えられる。よってイメージとしては圏$\cat{C}$の外側$\cat{C\times C}$で定義した対象の積もどき$[A,B]$を圏$\cat{C}$に挿入する操作が関手$(-\times-)$と考えられる。ただし、双積関手で写された対象が積の普遍性を満たすかどうかは現段階では説明できない。
	...
	\begin{prop}[射影射の自然性]
		...
		この自然変換を\[\natf{\pi_L}{(-\times -)}{\Pi_{LC}}{C\times C}{C}\]とし、成分を\[(\pi_L)_{A\times B}=\pi_{LA}\]とする。
