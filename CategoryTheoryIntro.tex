...
\begin{document}
  ...
  \author{@otamu32}
	...
	\tableofcontents
	\section{はじめに}
	本資料は数学基礎論や計算機科学で特に使われるデカルト閉圏を目標に解説していく。
  ...
  関手の射集合を厳密に表記すると$\mor{F_{A,B}}{\arset{C}{A}{B}}{\arset{D}{FA}{FB}}$のように添字がつくが、これも少し複雑ではあるが実際に自然変換になる。
	...
	\begin{define}[関手圏]
		...
		\begin{quote}
			\begin{description}
				\item[対象] $\obj{\funccat{C}{D}}$を、圏$\cat{C}$から圏$\cat{D}$への関手全体の集合とする。
  ...
  \begin{prop}[$A\times 1\cong A$は$A$に対して自然である]
    
  \end{prop}
  \begin{prop}[$\arset{C}{X}{B}\cong\arset{C}{X}{B'}$は$X$に対して自然である]

  \end{prop}
  \begin{prop}[同型と射集合の同型]

  \end{prop}
  \begin{prop}[$\arset{C}{X}{A\times B}\cong \arset{C}{X}{A}\times \arset{C}{X}{B}$は$X$に対して自然である]
  \end{prop}
