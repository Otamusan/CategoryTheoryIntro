\subsection{自然同型}\label{chap-7.2-natural-isomorphism}
  次に自然変換や関手を用いた量化された同型である自然同型を見ていく。
  \begin{define}[自然同型(関手の同型)]\label{def-natural-isomorphism}
  ...
  \begin{prop}[$A\times 1\cong A$の自然性]\label{prop-naturality-of-a-and-a1}
  ...
  \end{proof}
  \begin{prop}[$\arset{C}{X}{B}\cong\arset{C}{X}{B'}$の自然性]\label{prop-naturality-of-isomorphic-preserved-by-hom-functor}
  ...
  \begin{prop}[$\arset{C}{X}{A\times B}\cong \arset{C}{X}{A}\times \arset{C}{X}{B}$の自然性]\label{prop-naturality-of-isomorphic-by-product-and-hom-functor}~\\
    $A\times B$が$A,B$における積対象$\iff\arset{C}{X}{A\times B}\cong \arset{C}{X}{A}\times \arset{C}{X}{B}$が$X$に対して自然
  \end{prop}
  \begin{prop}[$\arset{C}{X}{1}\cong I$の自然性]\label{prop-naturality-of-isomorphic-by-terminal-object-and-hom-functor}~\\
